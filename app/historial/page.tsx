        ...
        <div className="mt-16">
          ...
          <div className="mx-auto max-w-7xl sm:px-2 lg:px-8">
            <div className="mx-auto max-w-2xl space-y-8 sm:px-4 lg:max-w-4xl lg:px-0">
              {isLoading
                ...
                : orders.length > 0
                  ? orders.map((order, index) => (
                      <OrderCard key={order.id || index} order={order as unknown as Order} />
                    ))
                  : !isLoading && <div className="text-gray-500">No tienes órdenes recientes</div>}
              ...
              {isFetchingNextPage &&
                [...Array(2)].map((_, index) => <OrderCardSkeleton key={`fetching-${index}`} />)}
            </div>
          </div>
        </div>
      </div>
