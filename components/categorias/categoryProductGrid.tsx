...
export default function CategoryProductGrid({
  ...
  const CATEGORIES = category || subCategory || niche;
  ...
  const { data: categoryData, isLoading: loadingSeo } = getCategoryBySlug(
    CATEGORIES?.toString() ?? '',
  );
  ...
  const seoTitle = categoryData
    ? `${categoryData.seo.title} | Adan Farmacia`
    : `${CATEGORIES ? `Búsqueda: ${CATEGORIES}` : 'Productos'} | Adan Farmacia`;

  const seoDescription = categoryData
    ? categoryData.seo.description ||
      `Explora nuestra selección de ${categoryData.seo.title.toLowerCase()}`
    : `Resultados de búsqueda para "${CATEGORIES}" en Adan Farmacia`;
